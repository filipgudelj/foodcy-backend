 ...
 */
final class Version20220615131917 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__recipe AS SELECT id, author_id, title, ingredients, steps, company FROM recipe');
        $this->addSql('DROP TABLE recipe');
        $this->addSql('CREATE TABLE recipe (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, author_id INTEGER DEFAULT NULL, title VARCHAR(255) NOT NULL, ingredients CLOB NOT NULL, steps CLOB NOT NULL, company VARCHAR(255) DEFAULT NULL, CONSTRAINT FK_DA88B137F675F31B FOREIGN KEY (author_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO recipe (id, author_id, title, ingredients, steps, company) SELECT id, author_id, title, ingredients, steps, company FROM __temp__recipe');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__recipe AS SELECT id, author_id, title, ingredients, steps, company FROM recipe');
        $this->addSql('DROP TABLE recipe');
        $this->addSql('CREATE TABLE recipe (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, author_id INTEGER DEFAULT NULL, title VARCHAR(255) NOT NULL, ingredients CLOB NOT NULL, steps CLOB NOT NULL, company VARCHAR(255) DEFAULT NULL)');
        $this->addSql('INSERT INTO recipe (id, author_id, title, ingredients, steps, company) SELECT id, author_id, title, ingredients, steps, company FROM __temp__recipe');
        $this->addSql('DROP TABLE __temp__recipe');
        $this->addSql('CREATE INDEX IDX_DA88B137F675F31B ON recipe (author_id)');
    }
