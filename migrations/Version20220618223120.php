 ...
 */
final class Version20220618223120 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__recipe AS SELECT id, author_id, image_id, title, ingredients, steps, company FROM recipe');
        ...
        $this->addSql('INSERT INTO recipe (id, author_id, image_id, title, ingredients, steps, company) SELECT id, author_id, image_id, title, ingredients, steps, company FROM __temp__recipe');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('DROP INDEX UNIQ_DA88B1373DA5256D');
        $this->addSql('DROP INDEX IDX_DA88B137F675F31B');
        $this->addSql('CREATE TEMPORARY TABLE __temp__recipe AS SELECT id, image_id, author_id, title, ingredients, steps, company FROM recipe');
        $this->addSql('DROP TABLE recipe');
        $this->addSql('CREATE TABLE recipe (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, image_id INTEGER DEFAULT NULL, author_id INTEGER DEFAULT NULL, title VARCHAR(255) NOT NULL, ingredients CLOB NOT NULL, steps CLOB NOT NULL, company CLOB DEFAULT NULL)');
        $this->addSql('INSERT INTO recipe (id, image_id, author_id, title, ingredients, steps, company) SELECT id, image_id, author_id, title, ingredients, steps, company FROM __temp__recipe');
        $this->addSql('DROP TABLE __temp__recipe');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_DA88B1373DA5256D ON recipe (image_id)');
        $this->addSql('CREATE INDEX IDX_DA88B137F675F31B ON recipe (author_id)');
    }
